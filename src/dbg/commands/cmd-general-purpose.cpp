...
static bool ReadWriteVariable(const char* varname, const std::function<bool(duint*, int)> & callback)
{
    ...
    if(!isvar || !valtostringfromvalue(varname, set_value, true))
    {
        duint value;
...
bool cbInstrPop(int argc, char* argv[])
{
    ...
    if(argc > 1)
    {
        if(!valtostringfromvalue(argv[1], value, false))
            return false;
    }
    return true;
}
...
bool cbInstrPopcnt(int argc, char* argv[])
{
    ...
    else
    {
        ...
        if(!isvar || !valtostringfromvalue(argv[1], arg, true))
        {
            duint value;
...
bool cbInstrMov(int argc, char* argv[])
{
    ...
    else
    {
        ...
        if(!isvar)
            isvar = vargettype(argv[1], 0);
        if(!isvar || !valtostringfromvalue(argv[1], set_value, true))
        {
            duint value;
            if(valfromstring(argv[1], &value)) //if the var is a value already it's an invalid destination
            {
...
#ifdef _WIN64
        if(!valtostringfromvalue(dstText.c_str(), context.Opmask[registerindex], false))
        {
            goto InvalidDest;
        }
#else // x86
...
bool cbInstrKmovd(int argc, char* argv[])
{
    ...
    else if(isKregisters[0])
    {
        ...
        if(!valtostringfromvalue(dstText.c_str(), DWORD(context.Opmask[registerindex]), false))
        {
